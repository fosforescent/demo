...
import { getAllOfNode, getNameNode, getTerminalNode, getUnitNode, getRootInstructionNode, getWorkflowInstructionNode, getFolderNode } from './node-factory'
...
export class Store implements IStore {
  ...
  init (): void {
    ...
    this.folderAddress = getFolderNode(this).getAddress()
    this.rootInstructionAddress = getRootInstructionNode(this).getAddress()
    ...
    const core = this.create([
      [this.nameAddress, this.create('core').getAddress()],
      [this.unitAddress, this.allOfAddress],
      [this.unitAddress, workflowAddress],
      [this.unitAddress, this.nameAddress],

    ]).getAddress()

    const root = this.create([
      [this.nameAddress, this.create('root').getAddress()],
      [this.folderAddress, core]

    ]).getAddress()
    this.rootsHistory.unshift(root)
  }
  ...
  toJSON (){
    ...
    console.log('exttable saved', [...this.externalData.keys()].length, [...this.externalData.values()].map(x => JSON.stringify(x.getValue())))
    const data = {
  ...
  fromJSONObj (json: any) {
    // console.log("test")
    try {
      // console.log("test")
      const [dataString, version] = json
      // console.log("test")
      // console.log('fromJSON', dataString)
